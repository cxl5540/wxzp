...
export default {
 ...
 this.getProData();
   ...
   methods:{
    ...
    editPro:function(index){   //编辑工作经验
       let proTable= this.proList[0][index];
       this.$router.push({name:'Editpro',query:{ editTable:proTable,type:'edit',id:index,objid:this.$route.query.objid} });
    },
    pro_add:function(){    //增加工作经验
         this.$router.push({name:'Editpro',query: { type:'add',objid:this.$route.query.objid} }); 
