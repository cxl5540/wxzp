...
export default {
 ...
 if(this.certList[0].length==1){
      ...
      this.$dialog.confirm({
            ...
            opts: () => {
                 let award_id = this.certList[0][index].award_id;
                  ...
                  let _this = this;
                  this.$http.post(_const.requestUrl+'/v1/userrecord/delAward',delparms).then(function ( res ){
    ...
    edit:function(index){   //编辑证书
       ...
       this.$router.push({name:'Editcertsend',query:{ editTable:certList,type:'edit',id:index,newbase_id:this.newbase_id} });
    },
    cert_add:function(){    //增加证书
         this.$router.push({name:'Editcertsend',query: { type:'add',id:this.id,newbase_id:this.newbase_id} }); 
