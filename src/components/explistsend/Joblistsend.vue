<template>
  <div>
    ...
    <div v-show='show' style="text-align: center;margin-top: 100px">
      <p style="font-size: 16px;color: #999">您还未添加工作经历11~~~</p>
  ...
  mounted () {
   ...
   methods:{
         ...
         return;
          ...
          this.$dialog.confirm({
            ...
            opts: () => {
                ...
                let _this = this;
                this.$http.post(_const.requestUrl+'/v1/userrecord/delWork',delparms).then(function ( res ){
    ...
    editJob:function(index,e){   //编辑工作经验
       ...
       this.$router.push({name:'Editjobsend',query:{ jobTable:jobTable,type:'edit',id:index,newbase_id:this.newbase_id} });

    },
    job_add:function(){    //增加工作经验
         this.$router.push({name:'Editjobsend',query: { type:'add',id:this.id,newbase_id:this.newbase_id} }); 
      },
