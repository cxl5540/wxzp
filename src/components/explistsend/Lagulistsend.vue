 ...
 this.findResume();
   ...
   methods:{
      ...
      this.$dialog.confirm({
            ...
            opts: () => {
               let language_id = this.laguList[0][index].language_id;
                ...
                let _this = this;
                this.$http.post(_const.requestUrl+'/v1/userrecord/delLanguage',delparms).then(function ( res ){
    ...
    edit:function(index){   //编辑工作经验
       ...
       this.$router.push({name:'Editlagusend',query:{ editTable:laguList,type:'edit',id:index,newbase_id:this.newbase_id} });
    },
    trian_add:function(){    //增加工作经验
         this.$router.push({name:'Editlagusend',query: { type:'add',id:this.id,newbase_id:this.newbase_id} }); 
