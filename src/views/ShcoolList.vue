<template>
 <div class="sclist" >
    ...
    <div style="margin-top: 52px;">
     <img src="../assets/icon/招聘.png" width="100%">
    </div>
       ...
       <scroller :on-refresh="refresh"
                  :on-infinite="infinite"               
                   ...
                   style="margin-top:366px;"
                    ...
                    >
      <div class="SchollList" style="background-color: #fff" v-for="(item, index) in SchoolList" :key="index">
        <!-- <router-link :to="'/shcoolListDetail/'+item.id"> -->
        <router-link :to="{path:'/shcoolListDetail', query:{id:item.id,calss:'2'}}">  
            <div style="border-bottom: 1px solid #eee;padding: 15px 15px;">
               ...
               <div style="display: flex;justify-content: space-between; display: -webkit-flex;">
            ...
            let selcvalue=item.job_address;
             ...
             let _this=this;
              this.$http.post(_const.requestUrl+'/v1/jobs/jobslist',serchParms).then(function ( res ) {
                console.log(res);
               _this.SchoolList = res.list.data;
                for(var i=0;i<_this.SchoolList.length;i++){
                    _this.SchoolList[i].create_time=_this.getLocalTime( _this.SchoolList[i].create_time);
                      }
               });
              setTimeout(function(){
                 $('#adrlsit').slideUp();
            ...
            getSchoolList () {                     //列表
                ...
                this.$http.post(_const.requestUrl+'/v1/jobs/jobslist',params).then(function ( res ) {
                  let reponData=res.list.data;          
                    ...
                    if(_this.page==1){
                       _this.SchoolList = reponData;                       

                     }else{
                       for(var i=0;i<reponData.length;i++){
                        _this.SchoolList.push(reponData[i]);
                       }
                   }
...
.nav{
  ...
  ._v-content{
    transform: translate3d(0px, -900px, 0px) scale(1);
  }
  .yd-navbar{
    position: fixed;
    top: 0;
